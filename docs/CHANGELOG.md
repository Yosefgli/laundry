...
- Removed 'New Order' nav tab; disabled dashboard button when session active; added cancel-session confirmation dialog; orders board shows all active orders (no 20-cap); barcode scan shows inactive orders as view-only; added missing translations (confirm_final, add_next_bag, bags, view_only, cancel_confirm); added feed spacing between order+payment barcodes on receipt.
...
- [2026-06-10] Added /admin/reports with date/service/color filters + CSV/PDF export; renamed OPS→POS throughout; reduced 'by Chabad' text size; removed VOID from status dropdown; added terms_accepted checkbox on customer summary + DB column; replaced balloon animation with soap bubbles; added pricing page instruction banner + redesigned service cards as info panels; PWA manifest + icon added.
